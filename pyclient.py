...
#from pynput import keyboardexit
...
print("The stage are  ----> : ", arguments.stage)

# Calling constructor of the Driver class from the driver file  (imp)

d = driver.Driver(arguments.stage)

print("The object is  ---------> : ", d)

while not shutdownClient:
    while True:
        print('Sending id to server: ', arguments.id)

        # Calling init function of the Driver class from the driver file (imp)
    ...
    while True:
        ...
        if buf != None and buf.decode().find('***shutdown***') >= 0:
            # Calling onShutDown function of the Driver class from driver file (imp)
            d.onShutDown()
            shutdownClient = True
        ...
        if buf != None and buf.decode().find('***restart***') >= 0:
            # Calling onRestart function of the Driver class from driver file (imp)
            d.onRestart()
        ...
        if currentStep != arguments.max_steps:
            if buf != None:
                # Calling drive function of the Driver class from driver file (imp)
...
print("The number of times pressed is : ", d.checkPress())
d.Display()
